...
do
	...
	'-L')
		...
		then
			...
			echo "// This file is generated by ./config.sh --make-config.mak"
			echo "static char bincfg[] ="
			echo "  \"\0\nCFG: strings FILE | grep ^CFG~ | sed -e 's/^CFG~//' | openssl enc -d -base64 | gzip -d\n\""
			gzip -9 < config.mak | $OPENSSL enc -base64 | while read LINE
			do
				printf "  \"CFG~%s\\\\n\"\n" "$LINE"
			done
			echo "  ;"
	...
	'-m'|'--make-config.mak')
		...
		then
			# config.mak differs
			cat $TMPFILE > config.mak
			$0 -L > config.mak.h
		else
			$0 -L > $TMPFILE
			# config.mak.h differs
			cmp $TMPFILE config.mak.h >/dev/null 2>/dev/null
			[ $? != 0 ] && cat $TMPFILE > config.mak.h
		fi
		rm -rf $TMPFILE
		exit 0
	;;
